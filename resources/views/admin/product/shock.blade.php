...
<br>
<div class="container">
<form action="{{url('/shockaction')}}" method="POST" role="form">
{{ csrf_field() }}
    <div class="row">
      ...
      <div class="panel panel-default panel-table">
              <div class="panel-heading">
                <div class="row">
                  <div class="col-md-6">
                    <h3 class="panel-title">โช้ค</h3>
                  </div>
                  <div class="col-md-6">
                    <input type="search" class="light-table-filter" data-table="order-table" placeholder="ค้นหาโช้ค">
                    @if (Auth::user()->id==1)
                ...
                <table class="table table-striped table-bordered table-list order-table table  table-responsive">
                  <thead>
                    ...
                    <tbody>
                        <tr>
                          <td>
                            <input type="hidden" name="{{$value->shock_id}}">
                            <input type="checkbox" class="group" id="{{$value->shock_id}}" name="checkbox[]" value="{{$value->shock_id}}">
                          </td>
                          ...
                          <td>{{$value->price}}</td>
                          <td>{{$value->detail}}</td>
                          <td><a href="{{url('admin/editshock/')}}/{{$value->shock_id}}" class="btn btn-info"><span class="glyphicon glyphicon-edit"></span> แก้ไข</a></td>
                        </tr>
                    </tbody>
                    @endforeach
                </table>
