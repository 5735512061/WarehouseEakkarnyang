...
<br>
<div class="container">
<form action="{{url('/brakeaction')}}" method="POST" role="form">
{{ csrf_field() }}
    <div class="row">
      ...
      <div class="panel panel-default panel-table">
              <div class="panel-heading">
                <div class="row">
                  <div class="col-md-6">
                    <h3 class="panel-title">เบรก</h3>
                  </div>
                  <div class="col-md-6">
                    <input type="search" class="light-table-filter" data-table="order-table" placeholder="ค้นหาเบรก">
                    @if (Auth::user()->id==1)
                ...
                <table class="table table-striped table-bordered table-list order-table table table-responsive">
                  <thead>
                    ...
                    <tbody>
                        <tr>
                          <td>
                            <input type="hidden" name="{{$value->brake_id}}">
                            <input type="checkbox" class="group" id="{{$value->brake_id}}" name="checkbox[]" value="{{$value->brake_id}}">
                          </td>
                          ...
                          <td>{{$value->price}}</td>
                          <td>{{$value->detail}}</td>
                          <td><a href="{{url('admin/editbrake/')}}/{{$value->brake_id}}" class="btn btn-info"><span class="glyphicon glyphicon-edit"></span> แก้ไข</a></td>
                        </tr>
                    </tbody>
                    @endforeach
                </table>
