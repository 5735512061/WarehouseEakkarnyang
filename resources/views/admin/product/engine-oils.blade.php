...
<br>
<div class="container">
<form action="{{url('/oilaction')}}" method="POST" role="form">
{{ csrf_field() }}
    <div class="row">
      ...
      <div class="panel panel-default panel-table">
              <div class="panel-heading">
                <div class="row">
                  <div class="col-md-6">
                    <h3 class="panel-title">น้ำมันเครื่อง</h3>
                  </div>
                  <div class="col-md-6">
                    <input type="search" class="light-table-filter" data-table="order-table" placeholder="ค้นหาน้ำมันเครื่อง">
                    @if (Auth::user()->id==1)
              ...
              <div class="panel-body">
                <table class="table table-striped table-bordered table-list order-table table table-responsive">
                  <thead>
                    ...
                    @foreach($oils as $oil => $value)
                    <tbody>
                        <tr>
                          <td>
                            <input type="hidden" name="{{$value->oil_id}}">
                            <input type="checkbox" class="group" id="{{$value->oil_id}}" name="checkbox[]" value="{{$value->oil_id}}">
                          </td>
                          <td>{{$NUM_PAGE*($page-1) + $oil+1}}</td>
                          <td>{{$value->subcategory}}</td>
                          <td>{{$value->model}}</td>
                          <td>{{$value->size}}</td>
                          <td>{{$value->price}}</td>
                          <td>{{$value->detail}}</td>
                          <td><a href="{{url('admin/editoil/')}}/{{$value->oil_id}}" class="btn btn-info"><span class="glyphicon glyphicon-edit"></span> แก้ไข</a></td>
                        </tr>
                    </tbody>
                    @endforeach
                </table>
