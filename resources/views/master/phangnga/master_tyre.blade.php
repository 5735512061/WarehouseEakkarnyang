...
@section('content')
    ...
    <div class="page-container" style="font-family: 'Noto Sans Thai', sans-serif !important;">
        ...
        <div class="main-content">
            <center>
                <h2 style="color: #fff;">คลังสินค้า สาขาเมืองพังงา</h2>
            </center>
            <center>
                <div class="col-md-4 mt-3">
                    <a href="{{ url('/master/phangnga/create-tyre') }}" type="button" class="btn btn-primary"><i class="zmdi zmdi-plus"></i> เพิ่มสินค้าในคลัง</a>
                </div>
                <div class="col-md-8 mt-3" style="margin-bottom: 15px;">
                    <form class="form-header" action="{{ url('/master/phangnga/search') }}" method="POST">{{ csrf_field() }}
            ...
            @php
                $amount_michelin = DB::table('tyrephangngas')
                    ->where('category', 'MICHELIN')
                    ->sum('amount');
                $amount_bfg = DB::table('tyrephangngas')
                    ->where('category', 'BF Goodrich')
                    ->sum('amount');
                $amount_otani = DB::table('tyrephangngas')
                    ->where('category', 'OTANI')
                    ->sum('amount');
                $amount_maxxis = DB::table('tyrephangngas')
                    ->where('category', 'MAXXIS')
                    ->sum('amount');
                $amount_yoko = DB::table('tyrephangngas')
                    ->where('category', 'YOKOHAMA')
                    ->sum('amount');
                $amount_brid = DB::table('tyrephangngas')
                    ->where('category', 'BRIDGESTONE')
                    ->sum('amount');
                $amount_toyo = DB::table('tyrephangngas')
                    ->where('category', 'TOYO')
                    ->sum('amount');
                $amount_nitto = DB::table('tyrephangngas')
                    ->where('category', 'NITTO')
                    ->sum('amount');
                $amount_goodyear = DB::table('tyrephangngas')
                    ->where('category', 'GOODYEAR')
                    ->sum('amount');
                $amount_raiden = DB::table('tyrephangngas')
                    ->where('category', 'RAIDEN')
                    ->sum('amount');
                $amount_other = DB::table('tyrephangngas')
                ...
                $stock = DB::table('tyrephangngas')->where('stock','!=',0)->where('stock_required','ต้องสต๊อก')->sum('stock');
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        @if (Auth::user()->role == '1')
                                                            <td>
                                                                <a
                                                                    href="{{ url('/master/phangnga/edit/') }}/{{ $value->id }}">
                                                                    <i class="fa fa-pencil-square"
                                                                        style="color:blue;"></i>
                                                                </a>
                                                                <button
                                                                    formaction="{{ url('/master/phangnga/tyredelete/') }}/{{ $value->id }}"
            ...
            <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel"
                ...
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{ url('/master/phangnga/delete-tyre') }}" method="POST"
            ...
            <div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel"
                ...
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{ url('/master/phangnga/add-tyre') }}" method="POST"
                            enctype="multipart/form-data" autocomplete="off">@csrf
                            <div class="modal-body">
                                <input type="text" class="form-control" name="amount"
