...
@section('content')
    ...
    <div class="page-container" style="font-family: 'Noto Sans Thai', sans-serif !important;">
        ...
        <div class="main-content">
            <center>
                <h2 style="color: #fff;">คลังสินค้า สาขาบายพาส</h2>
            </center>
            <center>
            ...
            @php
                $amount_michelin = DB::table('tyrebyparts')
                    ->where('category', 'MICHELIN')
                    ->sum('amount');
                $amount_bfg = DB::table('tyrebyparts')
                    ->where('category', 'BF Goodrich')
                    ->sum('amount');
                $amount_otani = DB::table('tyrebyparts')
                    ->where('category', 'OTANI')
                    ->sum('amount');
                $amount_maxxis = DB::table('tyrebyparts')
                    ->where('category', 'MAXXIS')
                    ->sum('amount');
                $amount_yoko = DB::table('tyrebyparts')
                    ->where('category', 'YOKOHAMA')
                    ->sum('amount');
                $amount_brid = DB::table('tyrebyparts')
                    ->where('category', 'BRIDGESTONE')
                    ->sum('amount');
                $amount_toyo = DB::table('tyrebyparts')
                    ->where('category', 'TOYO')
                    ->sum('amount');
                $amount_nitto = DB::table('tyrebyparts')
                    ->where('category', 'NITTO')
                    ->sum('amount');
                $amount_goodyear = DB::table('tyrebyparts')
                    ->where('category', 'GOODYEAR')
                    ->sum('amount');
                $amount_raiden = DB::table('tyrebyparts')
                    ->where('category', 'RAIDEN')
                    ->sum('amount');
                $amount_other = DB::table('tyrebyparts')
                ...
                $stock = DB::table('tyrebyparts')->where('stock','!=',0)->where('stock_required','ต้องสต๊อก')->sum('stock');
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        @if (Auth::user()->role == '1')
                                                            <td>
                                                                <a
                                                                    href="{{ url('/master/bypart/edit/') }}/{{ $value->id }}">
                                                                    <i class="fa fa-pencil-square"
                                                                        style="color:blue;"></i>
                                                                </a>
                                                                <button
                                                                    formaction="{{ url('/master/bypart/tyredelete/') }}/{{ $value->id }}"
        ...
        <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel"
            ...
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ url('/master/bypart/delete-tyre') }}" method="POST" enctype="multipart/form-data"
                        autocomplete="off">@csrf
                        <div class="modal-body">
                            <input type="text" class="form-control" style="height: calc(1.5rem)" name="amount"
        ...
        <div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel"
            ...
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ url('/master/bypart/add-tyre') }}" method="POST" enctype="multipart/form-data"
                        autocomplete="off">@csrf
                        <div class="modal-body">
                            <input type="text" class="form-control" style="height: calc(1.5rem)" name="amount"
