    ...
    @include("/master/master_navbar_mobile")  
        ...
        <div class="page-container">
            ...
            <div class="main-content">
                <center><h2 style="color: #fff;">คลังสินค้า สาขาเจ้าฟ้า</h2></center>
                <center>
                ...
                @php
                    $amount_michelin = DB::table('tyrechaofas')->where('category','MICHELIN')->sum('amount');
                    $amount_bfg = DB::table('tyrechaofas')->where('category','BF Goodrich')->sum('amount');
                    $amount_otani = DB::table('tyrechaofas')->where('category','OTANI')->sum('amount');
                    $amount_maxxis = DB::table('tyrechaofas')->where('category','MAXXIS')->sum('amount');
                    $amount_yoko = DB::table('tyrechaofas')->where('category','YOKOHAMA')->sum('amount');
                    $amount_brid = DB::table('tyrechaofas')->where('category','BRIDGESTONE')->sum('amount');
                    $amount_toyo = DB::table('tyrechaofas')->where('category','TOYO')->sum('amount');
                    $amount_nitto = DB::table('tyrechaofas')->where('category','NITTO')->sum('amount');
                    $amount_goodyear = DB::table('tyrechaofas')->where('category','GOODYEAR')->sum('amount');
                    $amount_raiden = DB::table('tyrechaofas')->where('category','RAIDEN')->sum('amount');
                    $amount_other = DB::table('tyrechaofas')->where('category','อื่นๆ')->sum('amount');
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    @if(Auth::user()->role == "1")
                                                    <td>
                                                        <a href="{{url('/master/chaofa/edit/')}}/{{$value->id}}">
                                                            <i class="fa fa-pencil-square" style="color:blue;"></i>
                                                        </a>
                                                        <button formaction="{{url('/master/chaofa/tyredelete/')}}/{{$value->id}}" onclick="return confirm('Are you sure to delete ?')">
                            ...
                            <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-sm" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{url('/master/chaofa/delete-tyre')}}" method="POST" enctype="multipart/form-data" autocomplete="off">@csrf
                                        <div class="modal-body">
                            ...
                            <div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-sm" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{url('/master/chaofa/add-tyre')}}" method="POST" enctype="multipart/form-data" autocomplete="off">@csrf
                                        <div class="modal-body">
