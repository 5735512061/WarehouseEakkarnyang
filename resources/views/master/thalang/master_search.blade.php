    ...
    @include("/master/master_navbar_mobile")  
        ...
        <div class="page-container">
            ...
            <div class="main-content">
                <center><h2 style="color: #fff;">คลังสินค้า สาขาถลาง</h2></center>
                <center>
                    <div class="col-md-8 mt-3" style="margin-bottom: 15px;">
                        <form class="form-header" action="{{url('/master/thalang/search')}}" method="POST">{{ csrf_field() }}
                ...
                @php
                    $amount_michelin = DB::table('tyrethalangs')->where('category','MICHELIN')->sum('amount');
                    $amount_bfg = DB::table('tyrethalangs')->where('category','BF Goodrich')->sum('amount');
                    $amount_otani = DB::table('tyrethalangs')->where('category','OTANI')->sum('amount');
                    $amount_maxxis = DB::table('tyrethalangs')->where('category','MAXXIS')->sum('amount');
                    $amount_yoko = DB::table('tyrethalangs')->where('category','YOKOHAMA')->sum('amount');
                    $amount_brid = DB::table('tyrethalangs')->where('category','BRIDGESTONE')->sum('amount');
                    $amount_toyo = DB::table('tyrethalangs')->where('category','TOYO')->sum('amount');
                    $amount_nitto = DB::table('tyrethalangs')->where('category','NITTO')->sum('amount');
                    $amount_goodyear = DB::table('tyrethalangs')->where('category','GOODYEAR')->sum('amount');
                    $amount_raiden = DB::table('tyrethalangs')->where('category','RAIDEN')->sum('amount');
                    $amount_other = DB::table('tyrethalangs')->where('category','อื่นๆ')->sum('amount');
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    @if(Auth::user()->role == "1")
                                                    <td>
                                                        <a href="{{url('/master/thalang/edit/')}}/{{$value->id}}">
                                                            <i class="fa fa-pencil-square" style="color:blue;"></i>
                                                        </a>
                                                        <button formaction="{{url('/master/thalang/tyredelete/')}}/{{$value->id}}" onclick="return confirm('Are you sure to delete ?')">
                            ...
                            <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-sm" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{url('/master/thalang/delete-tyre')}}" method="POST" enctype="multipart/form-data" autocomplete="off">@csrf
                                        <div class="modal-body">
                            ...
                            <div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-sm" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{url('/master/thalang/add-tyre')}}" method="POST" enctype="multipart/form-data" autocomplete="off">@csrf
                                        <div class="modal-body">
