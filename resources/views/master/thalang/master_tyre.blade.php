...
@section('content')
    ...
    <div class="page-container" style="font-family: 'Noto Sans Thai', sans-serif !important;">
        ...
        <div class="main-content">
            <center>
                <h2 style="color: #fff;">คลังสินค้า สาขาถลาง</h2>
            </center>
            <center>
                <div class="col-md-4 mt-3">
                    <a href="{{ url('/master/thalang/create-tyre') }}" type="button" class="btn btn-primary"><i class="zmdi zmdi-plus"></i> เพิ่มสินค้าในคลัง</a>
                </div>
            ...
            @php
                $amount_michelin = DB::table('tyrethalangs')
                    ->where('category', 'MICHELIN')
                    ->sum('amount');
                $amount_bfg = DB::table('tyrethalangs')
                    ->where('category', 'BF Goodrich')
                    ->sum('amount');
                $amount_otani = DB::table('tyrethalangs')
                    ->where('category', 'OTANI')
                    ->sum('amount');
                $amount_maxxis = DB::table('tyrethalangs')
                    ->where('category', 'MAXXIS')
                    ->sum('amount');
                $amount_yoko = DB::table('tyrethalangs')
                    ->where('category', 'YOKOHAMA')
                    ->sum('amount');
                $amount_brid = DB::table('tyrethalangs')
                    ->where('category', 'BRIDGESTONE')
                    ->sum('amount');
                $amount_toyo = DB::table('tyrethalangs')
                    ->where('category', 'TOYO')
                    ->sum('amount');
                $amount_nitto = DB::table('tyrethalangs')
                    ->where('category', 'NITTO')
                    ->sum('amount');
                $amount_goodyear = DB::table('tyrethalangs')
                    ->where('category', 'GOODYEAR')
                    ->sum('amount');
                $amount_raiden = DB::table('tyrethalangs')
                    ->where('category', 'RAIDEN')
                    ->sum('amount');
                $amount_other = DB::table('tyrethalangs')
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        @if (Auth::user()->role == '1')
                                                            <td>
                                                                <a
                                                                    href="{{ url('/master/thalang/edit/') }}/{{ $value->id }}">
                                                                    <i class="fa fa-pencil-square"
                                                                        style="color:blue;"></i>
                                                                </a>
                                                                <button
                                                                    formaction="{{ url('/master/thalang/tyredelete/') }}/{{ $value->id }}"
            ...
            <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel"
                ...
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{ url('/master/thalang/delete-tyre') }}" method="POST"
            ...
            <div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel"
                ...
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{ url('/master/thalang/add-tyre') }}" method="POST"
                            enctype="multipart/form-data" autocomplete="off">@csrf
                            <div class="modal-body">
                                <input type="text" class="form-control" name="amount"
